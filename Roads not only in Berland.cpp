...
using namespace std;
// ————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
const int N = 1e3 + 5; // maximum number of elements in DSU
struct DSU
{
...
void TC() {
    ...
    for (int i = 0; i < n - 1; ++i) {
        int a , b;
        cin >> a >> b;
        if (not dsu.join(--a , --b)) removed.pb({a , b});
    }
    cout << removed.size() << nl;
    for(auto [a , b] : removed){
        cout << a + 1 << sp << b + 1 << sp << dsu.sets[0] + 1 << sp << dsu.sets[1] + 1 << nl;
        dsu.join(dsu.sets[0] , dsu.sets[1]);
    }
}
void file()
{
