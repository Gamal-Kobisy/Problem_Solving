...
// LINK : https://codeforces.com/group/zZYq310uLQ/contest/699485/problem/C
#include <bits/stdc++.h>
using namespace std;
#define ll long long
...
void TC() {
    ...
    {
        mask.set(i);
        if (isPrime(i)) prime_mask.set(i);
    }

    dfs(1 , -1);
    segTree seg(timer + 1);
    vector<int>flat(timer + 1);
    for (int i = 1 ; i <= n ; ++ i) {
        flat[tin[i]] = val[i];
    }
    seg.build(flat);
    ...
    while (q --)
    {
        ...
        {
            cin >> u;
            cout << (seg.query(tin[u] , tout[u]).b & prime_mask).count() << nl;
        }
    }
}
void file()
