...
struct Aho {
    ...
    vector<int> match(string &s, int numPat) {
        vector<int> ret(numPat);
        int cur = 0, i = 0;
        for (char c: s) {
            cur = nxtF(cur, c);
    ...
    vector<bool> checkExistence(string &s, int numPat) {
        vector<bool> visited(Trie.size(), 0);
        int cur = 0;
        for (char c: s) {
            cur = nxtF(cur, c);
            visited[cur] = 1;
...
//            int u = order[i];
//            if (visited[u]) visited[Trie[u].fail] = 1;
//        }
        vector<bool> found(numPat, 0);
        for (int u = 0; u < Trie.size(); ++u)
            if (~Trie[u].id) found[Trie[u].id] = visited[u];
        return found;
    }
