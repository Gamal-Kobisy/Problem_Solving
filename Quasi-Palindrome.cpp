...
// LINK : https://codeforces.com/problemset/gymProblem/104505/C
...
void TC() {
    ...
    ll ans = 0;
    auto ok = [&](int l , int r){
        if(h.getPrefix(l , r) == h.getSuffix(l , r)) return true;
        int lo = 1 , hi = r - l + 1 , lcp = 0;
        while(lo <= hi){
            int md = (lo + hi) / 2;
            if(h.getPrefix(l , l + md - 1) == h.getSuffix(r - md + 1, r)){
                lcp = md;
                lo = md + 1;
            }else{
                hi = md - 1;
        ...
        return h.getPrefix(l + lcp + 1 , r - lcp - 1) == h.getSuffix(l + lcp + 1 , r - lcp - 1);
    };
