...
mat operator*(const mat &a, const mat &b) {
    ...
    mat ret(r1, row(c2 , infLL));
    for (int i = 0; i < r1; i++)
        for (int j = 0; j < c2; j++)
            for (int k = 0; k < r2; k++){
                if(a[i][k] == infLL or b[k][j] == infLL) continue;
...
mat operator^(const mat &a, ll k) {
    mat ret(a.size(), row(a.size() , infLL));
    for (int i = 0; i < a.size(); i++)ret[i][i] = 0;
