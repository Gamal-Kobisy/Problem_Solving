...
// LINK : https://codeforces.com/problemset/problem/615/C
#include <bits/stdc++.h>
using namespace std;
#define ll long long
...
using namespace std;
// ————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
const int N = 2100 + 5, M = 1e3, LOG = 20, inf = 0x3f3f3f3f;
...
void TC() {
    ...
    while(l < t.size()) {
        ...
        for (int i = 0; i < s.size(); ++i) {
            ...
            int len = 0;
            while (i + len < s.size() and l + len < t.size() and s[i + len] == t[l + len]) {
                len++;
            }
            if (len > mx_len) {
                mx_len = len;
            ...
            len = 0;
            while (i - len >= 0 and l + len < t.size() and s[i - len] == t[l + len]) {
                len++;
            }
            if (len > mx_len) {
                mx_len = len;
        ...
        if (mx_len == 0) return void(cout << -1 << nl);
    ...
    cout << subs.size() << nl;
    for(auto [l , r] : subs) cout << l << sp << r << nl;
}
void file()
{
#ifndef ONLINE_JUDGE
