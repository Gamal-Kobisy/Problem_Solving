...
void TC() {
    ...
    while(q--){
        ...
        if(h.getPrefix(l1, r1) == h.getPrefix(l2, r2)) {
            cout << "SIMILAR" << nl;
            continue;
        }
        int lo = 1 , hi = min(len1 , len2) , lcp = 0;
        while(lo <= hi){
            int mid = (lo + hi) / 2;
            if(h.getPrefix(l1, l1 + mid - 1) == h.getPrefix(l2, l2 + mid - 1)){
                lcp = mid;
                lo = mid + 1;
            }else{
                hi = mid - 1;
        ...
        if(h.getPrefix(l1 + lcp + 1, r1) == h.getPrefix(l2 + lcp + 1, r2)) {
            cout << "SIMILAR" << nl;
        } else {
            cout << "DIFFERENT" << nl;
        }
