...
using namespace std;
// ————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
const int N = 2e5 + 5; // maximum number of elements in DSU
struct DSU
{
...
void TC() {
    ...
    vector<pii>edges;
    for (int i = 1; i < n; ++i) {
        if(dsu1.find(i) != dsu1.find(0) and dsu2.find(i) != dsu2.find(0)){
            dsu1.join(0 , i);
            dsu2.join(0 , i);
            edges.pb({1 , i + 1});
        }
    ...
    int idx1 = 0 , idx2 = 0;
    while(idx1 < dsu1.ncmp and idx2 < dsu2.ncmp){
        if(dsu1.sets[idx1] == dsu1.find(0)){
            idx1++;
            continue;
        }
        if(dsu2.sets[idx2] == dsu2.find(0)) {
            idx2++;
            continue;
        }
        edges.pb({dsu1.sets[idx1] + 1 , dsu2.sets[idx2] + 1});
        ++idx1 , ++idx2;
    }
    cout << edges.size() << nl;
    for(auto [a , b] : edges){
